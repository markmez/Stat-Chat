...
def _resolve_unique_overlaps(conn, alias, canonical, dry_run):
    ...
    # Computed tables: the alias and canonical rows DISAGREE by design —
    # they're each computed from their half of the split game logs.
    # Wholesale-delete the alias rows; canonical regenerates correctly
    # from the unified data once detect_streaks.py runs post-merge.
    _COMPUTED_TABLES = {"current_form", "pitching_current_form"}
    if not dry_run:
        for tbl in _COMPUTED_TABLES:
            n = conn.execute(
    ...
    # Raw-data tables: discover every UNIQUE index that includes
    # `player_id` across all tables (other than computed) and resolve
    # conflicts on each. This avoids hand-coding the long list of
    # *_splits tables.
    table_rows = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table'"
    ).fetchall()
    for (tbl,) in table_rows:
        if tbl in _COMPUTED_TABLES:
            continue
        if tbl in {"players", "player_id_aliases"}:
            continue
        # Skip rebuildable computed tables — they get regenerated by
        # their own builders and the merge script doesn't touch them.
        if tbl in {"career_ranks", "career_franchise_ranks", "mlb_records",
                   "team_records", "historical_streaks", "record_progression",
                   "historic_moments", "historical_index"}:
            continue
        # Does the table have a player_id column?
        col_info = conn.execute(f"PRAGMA table_info({tbl})").fetchall()
        col_names = {r[1] for r in col_info}
        if "player_id" not in col_names:
            continue
        # Find UNIQUE indexes that include player_id.
        idx_rows = conn.execute(f"PRAGMA index_list({tbl})").fetchall()
        for idx_row in idx_rows:
            # PRAGMA index_list shape: (seq, name, unique, origin, partial)
            if not idx_row[2]:
                continue
            idx_name = idx_row[1]
