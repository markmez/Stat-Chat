...
def _resolve_unique_overlaps(conn, alias, canonical, dry_run):
    ...
    # Raw-data tables: UNIQUE conflicts come from byte-identical duplicate
    # rows (MSF cutover artifacts); resolve by deleting the redundant
    # alias row. Real value mismatches abort the pair.
    ...
    # Wholesale-delete the alias rows; the canonical's rows will be
    # correctly regenerated by detect_streaks.py once the game logs
    # are unified. Run that script (or the nightly pipeline) after
    # the merge for these tables to refresh.
    if not dry_run:
        for tbl in ("current_form", "pitching_current_form"):
            n = conn.execute(
