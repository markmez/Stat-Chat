...
# Tables computed from game_logs by pull_live_stats / detect_streaks.
# Treated as "delete alias rows wholesale" during merge; the post-merge
# pipeline run regenerates the canonical rows from unified game logs.
_COMPUTED_FROM_GAMELOGS = {
...
def _find_player_id_tables(conn):
    """Enumerate raw-data tables with a player_id column. Excludes
    rebuildables (their own builders fix them post-merge) and computed
    tables (deleted wholesale during the merge — regenerate from unified
    game logs)."""
    ...
    for (name,) in rows:
        ...
        if name in _REBUILD_AFTER_MERGE:
            continue
        if name in _COMPUTED_FROM_GAMELOGS:
            continue
...
def _resolve_unique_overlaps(conn, alias, canonical, dry_run):
    ...
    # Computed/derived tables: alias and canonical rows disagree by
    # design — each is computed from that id's half of the split data.
    # Wholesale-delete the alias rows; the post-merge pipeline run
    # (pull_live_stats + detect_streaks) regenerates canonical's rows
    # from the unified game logs.
    if not dry_run:
        for tbl in _COMPUTED_FROM_GAMELOGS:
            try:
                n = conn.execute(
                    f"SELECT COUNT(*) FROM {tbl} WHERE player_id = ?", (alias,)
                ).fetchone()[0]
            except sqlite3.OperationalError:
                continue  # table may not exist on this DB
            if n:
                conn.execute(
                    f"DELETE FROM {tbl} WHERE player_id = ?", (alias,)
                )
                deletions.append({"table": tbl, "deleted_count": n,
                                  "reason": "computed table — regenerated post-merge"})

    # Raw-data tables: walk every UNIQUE index that includes player_id
    # and resolve identical-duplicate conflicts. Computed tables and
    # rebuildables are excluded from `_find_player_id_tables` already.
    table_rows = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table'"
    ).fetchall()
    raw_tables = set(_find_player_id_tables(conn))
