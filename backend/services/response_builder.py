...
def build_team_overview(team_code: str, season: int) -> Optional[str]:
    """Combined hitting + pitching + record overview for a team.

    Used when a team query has no explicit hitting/pitching signal and no
    specific stat ("Diamondbacks stats", "AZ Diamondbacks 2026 stats").
    Previous behavior defaulted silently to a batting leaderboard, which
    made pitching invisible — even the follow-up suggestion pills only
    offered more batting angles.

    Returns a single response body containing:
      - Team full name + W-L record subtitle
      - Top 8 hitters (compact grid: AVG/HR/RBI/OPS)
      - Top 8 pitchers (compact grid: IP/ERA/K)
      - Suggestion pills covering both sides + record + opposing splits
    """
    conn = _get_db()
    try:
        full_name = _team_full_name(team_code)
        nickname = _team_nickname(team_code)
        team_filter_bat = "(s.team = ? OR s.team LIKE ? OR s.team LIKE ?)"
        team_params = [team_code, f"{team_code}/%", f"%/{team_code}"]

        cur = conn.cursor()

        # W-L record for the season (top-line context)
        cur.execute(
            "SELECT "
            "  SUM(CASE WHEN result='W' THEN 1 ELSE 0 END) AS w, "
            "  SUM(CASE WHEN result='L' THEN 1 ELSE 0 END) AS l "
            "FROM team_game_results "
            "WHERE team = ? AND season = ? AND COALESCE(gametype, 'regular') = 'regular'",
            (team_code, season),
        )
        wl_row = cur.fetchone()
        record_str = ""
        if wl_row and wl_row[0] is not None:
            w, l = int(wl_row[0] or 0), int(wl_row[1] or 0)
            if w + l > 0:
                pct = w / (w + l)
                record_str = f"{w}-{l} ({pct:.3f}".replace("0.", ".") + ")"

        # Top hitters — same shape as build_team_stats overview, but cap at 8
        cur.execute(
            "SELECT p.name, s.batting_avg, s.home_runs, s.rbi, s.ops "
            "FROM season_batting_stats s "
            "JOIN players p ON s.player_id = p.player_id "
            f"WHERE {team_filter_bat} AND s.season = ? AND s.plate_appearances >= 1 "
            "ORDER BY s.plate_appearances DESC LIMIT 8",
            tuple(team_params + [season]),
        )
        bat_rows = cur.fetchall()

        # Top pitchers by IP (roster order, no rate-stat qualifier)
        team_filter_pit = "(sp.team = ? OR sp.team LIKE ? OR sp.team LIKE ?)"
        cur.execute(
            "SELECT p.name, sp.era, sp.strikeouts, sp.ip_outs "
            "FROM season_pitching_stats sp "
            "JOIN players p ON sp.player_id = p.player_id "
            f"WHERE {team_filter_pit} AND sp.season = ? AND sp.ip_outs >= 1 "
            "ORDER BY sp.ip_outs DESC LIMIT 8",
            tuple(team_params + [season]),
        )
        pit_rows = cur.fetchall()

        if not bat_rows and not pit_rows:
            return None  # Nothing to show — caller may fall back

        parts = []
        subtitle = f"{season}" + (f" — {record_str}" if record_str else "")
        parts.append(f"**{full_name}** — {subtitle}\n")
        parts.append("[TIP]Tap a player name for their full profile.[/TIP]")
        ...
        if bat_rows:
            ...
            for i, row in enumerate(bat_rows):
                avg = _format_rate(str(row[1]))
                ops = _format_rate(str(row[4]))
                parts.append(f"ROW {i+1}. {row[0]}: {avg}, {row[2]}, {row[3]}, {ops}")
            parts.append("[/LEADERBOARD]")
        ...
        # Follow-up suggestions: offer angles the user didn't just see.
        # Skip the low-value "batting average" pill (avg was in the grid);
        # go for record, pitching detail, opposing splits, home/road.
        parts.append(f"\n[SUGGEST]{nickname} record {season}[/SUGGEST]")
        parts.append(f"[SUGGEST]{nickname} pitching leaders[/SUGGEST]")
        parts.append(f"[SUGGEST]{nickname} home record[/SUGGEST]")
        parts.append(f"[SUGGEST]{nickname} home runs leaders[/SUGGEST]")
        return "\n".join(parts)
    finally:
        conn.close()
