...
def _fetch_streaks_for_season(conn, name, season, performance="hot", career=False):
    """Fetch batting streaks with T1->T2->T3 fallback. Returns list of row tuples or [].
    career=True scans all seasons (season is ignored) for the most extreme stretches."""
...
def build_streak_list(name: str, performance: str, season: Optional[int] = None,
    ...
    try:
        ...
        rows = _fetch_streaks_for_season(conn, name, target_season, performance, career=career)
        if not rows:
        ...
        # League avg OPS for cold streak context (skip for career \u2014 mixed seasons)
        league_ops = None
...
def build_batter_vs_pitcher(batter: str, pitcher: str) -> Optional[str]:
    """Batter vs pitcher: head-to-head history first (game_split_logs h2h,
    current season), then a matchup profile — the batter's splits vs the
    pitcher's handedness and primary pitch mix (Mark 2026-08-31)."""
    from datetime import date as _date
    season = _date.today().year
    conn = _get_db()
    try:
        b = conn.execute("SELECT player_id, name FROM players WHERE name = ?",
                         (_sanitize(batter),)).fetchone()
        p = conn.execute("SELECT player_id, name, throws FROM players WHERE name = ?",
                         (_sanitize(pitcher),)).fetchone()
        if not b or not p:
            return None
        bid, bname = b
        pid, pname, throws = p
        ...
        h2h = conn.execute(
            "SELECT SUM(plate_appearances), SUM(at_bats), SUM(hits), SUM(doubles),"
            " SUM(triples), SUM(home_runs), SUM(walks), SUM(strikeouts),"
            " SUM(COALESCE(hit_by_pitch,0)), SUM(COALESCE(sacrifice_flies,0))"
            " FROM game_split_logs WHERE player_id = ? AND season = ?"
            " AND family = 'h2h' AND perspective = 'bat' AND split = ?",
            (bid, season, pid)).fetchone()

        hand = {"L": "left-handed", "R": "right-handed"}.get(throws or "", "")
        hand_split = "vs_LHP" if throws == "L" else "vs_RHP"
        platoon = conn.execute(
            "SELECT SUM(plate_appearances), SUM(hits), SUM(walks),"
            " SUM(COALESCE(hit_by_pitch,0)), SUM(COALESCE(sacrifice_flies,0)),"
            " SUM(at_bats), SUM(doubles), SUM(triples), SUM(home_runs)"
            " FROM game_split_logs WHERE player_id = ? AND season = ?"
            " AND family = 'platoon' AND perspective = 'bat' AND split = ?",
            (bid, season, hand_split)).fetchone()

        mix = conn.execute(
            "SELECT split, SUM(plate_appearances) pa FROM game_split_logs"
            " WHERE player_id = ? AND season = ? AND family = 'pitch_type'"
            " AND perspective = 'pitch' GROUP BY split ORDER BY pa DESC LIMIT 2",
            (pid, season)).fetchall()
        bat_vs_pitch = []
        for ptype, _pa in mix:
            r = conn.execute(
                "SELECT SUM(plate_appearances), SUM(hits), SUM(walks),"
                " SUM(COALESCE(hit_by_pitch,0)), SUM(COALESCE(sacrifice_flies,0)),"
                " SUM(at_bats), SUM(doubles), SUM(triples), SUM(home_runs)"
                " FROM game_split_logs WHERE player_id = ? AND season = ?"
                " AND family = 'pitch_type' AND perspective = 'bat' AND split = ?",
                (bid, season, ptype)).fetchone()
            bat_vs_pitch.append((ptype, r))
    finally:
        conn.close()
    ...
    def _ops(row):
        if not row or not row[0]:
            return None, 0
        pa, h, bb, hbp, sf, ab, d2, t3, hr = row
        pa, h, bb, hbp, sf, ab, d2, t3, hr = [x or 0 for x in (pa, h, bb, hbp, sf, ab, d2, t3, hr)]
        if not ab:
            return None, pa
    ...
    _PITCH_LABEL = {"4-Seam": "four-seam fastball", "2-Seam": "two-seamer",
                    "Sinker": "sinker", "Cutter": "cutter", "Slider": "slider",
                    "Curve": "curveball", "Change": "changeup", "Split": "splitter"}

    parts = [f"**{bname} vs {pname}** ({season})\n"]

    pa_h2h = (h2h[0] or 0) if h2h else 0
    if pa_h2h:
        pa, ab, h, d2, t3, hr, bb, so, hbp, sf = [x or 0 for x in h2h]
        avg = h / ab if ab else 0
        obp_den = ab + bb + hbp + sf
        obp = (h + bb + hbp) / obp_den if obp_den else 0
        slg = ((h + d2 + 2 * t3 + 3 * hr) / ab) if ab else 0
        parts.append("[STATGRID]")
        parts.append("HEADER: PA, AB, H, HR, BB, SO, AVG, OPS")
        parts.append(f"ROW {bname}: {pa}, {ab}, {h}, {hr}, {bb}, {so},"
                     f" {_f3(avg)}, {_f3(obp + slg)}")
        parts.append("[/STATGRID]")
        line = f"{bname} is {h}-for-{ab}"
        if hr:
            line += f" with {hr} home run{'s' if hr > 1 else ''}"
        line += f" against {pname} this season."
        parts.append(line)
    else:
        parts.append(f"{bname} hasn't faced {pname} this season.")

    profile = []
    p_ops, p_pa = _ops(platoon)
    if p_ops is not None and p_pa >= 20 and hand:
        profile.append(f"{pname} is {hand} — {bname} has a {_f3(p_ops)} OPS"
                       f" against {'lefties' if throws == 'L' else 'righties'}"
                       f" this season ({p_pa} PA).")
    mix_bits = []
    for ptype, row in bat_vs_pitch:
        o, mpa = _ops(row)
        if o is not None and mpa >= 15:
            mix_bits.append(f"a {_f3(o)} OPS against {_PITCH_LABEL.get(ptype, ptype)}s")
    if mix_bits and mix:
        lead = " and ".join(_PITCH_LABEL.get(m[0], m[0]) for m in mix[:2])
        profile.append(f"{pname} leans on his {lead}; {bname} is hitting"
                       f" {' and '.join(mix_bits)}.")
    if profile:
        parts.append("")
        parts.append("**The matchup profile:** " + " ".join(profile))

    parts.append(f"\n[SUGGEST]{bname} stats[/SUGGEST]")
    parts.append(f"[SUGGEST]{pname} stats[/SUGGEST]")
    return "\n".join(parts)
