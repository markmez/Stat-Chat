"""
Local stat definitions — zero API cost for "what is OPS?" queries.
Python port of StatDefinitions.swift.
"""
...
STAT_DEFINITIONS = {
    ...
    "OPS+": "OPS adjusted for league average. 100 = league average hitter, 150+ is elite. Lets you compare across different seasons. Note: Early in the season, OPS+ can fluctuate significantly as league averages stabilize.",
    ...
    "wOBA": "Weighted on-base average — a rate stat that weights each way of reaching base by its run value. Unlike OPS, it properly weights walks, singles, doubles, triples, and home runs by how much they contribute to run scoring. Scale is similar to OBP (~.300 is average, ~.400 is excellent).",
    "FIP": "Fielding independent pitching — estimates a pitcher's run prevention independent of the defense behind them. Uses only strikeouts, walks, HBP, and home runs — outcomes the pitcher controls. Scale matches ERA (~3.50 is average, under 3.00 is excellent).",
...
def lookup(abbrev: str):
    """Look up a stat definition by its abbreviation (case-insensitive)."""
    key = abbrev.strip()
    return STAT_DEFINITIONS.get(key) or STAT_DEFINITIONS.get(key.upper())
