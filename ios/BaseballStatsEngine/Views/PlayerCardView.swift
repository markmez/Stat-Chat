        ...
        do {
            ...
            gameLogs = data.map { g in
                GameLog(
                    ...
                    hitByPitch: g.hit_by_pitch ?? 0,
                    sacrificeFlies: g.sacrifice_flies ?? 0
                )
    ...
    private func recomputeFormStats(
        ...
        if let logs = gameLogs, !logs.isEmpty {
            ...
            for g in slice {
                ...
                totalHBP += g.hitByPitch; totalSF += g.sacrificeFlies
            }

            // Standard OBP formula — (H + BB + HBP) / (AB + BB + HBP + SF).
            // Previous code dropped HBP from the numerator and used PA as
            // the denominator, which produced .025+ OPS divergence from the
            // season aggregate when the slider was at max position.
            let avg = totalAB > 0 ? Double(totalH) / Double(totalAB) : 0
            let obpNum = Double(totalH + totalBB + totalHBP)
            let obpDen = Double(totalAB + totalBB + totalHBP + totalSF)
            let obp = obpDen > 0 ? obpNum / obpDen : 0
