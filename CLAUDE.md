...
**Plan-coverage guard (2026-08-07):** execute() refuses any result whose executor branch didn't declare it honors every populated cross-cutting plan dimension (since_date, end_date, team_code, team_context, league, player_name, split_context) — the query falls through to the LLM tiers (grounded) instead of shipping a wrong slice, logged as response_type `plan_dims_dropped` (= the demand-ranked wishlist for native executor intersections). When adding an executor or widening one, update its `honored` set beside the `result =` line. Calibrate with audit_routing — too-strict annotations show up as intercepted→miss regressions in minutes. Bonus discovered on day one: guard refusals can UN-SHADOW correct downstream parsers (day/night splits were being shadowed by wrong-slice leaderboard claims for months).
