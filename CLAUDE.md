...
- **Disable autocorrect on search fields** — iOS autocorrect mangles player names (e.g., "Bregman" → "Bergman", "Ohtani" → "Obtain"). Add `.autocorrectionDisabled()` to all 4 TextFields: `HomeView` (line 110), `TeamCardView` (line 216), `PlayerCardView` (line 327), `ResultsView` (line 200). `PlayerNameMatcher` fuzzy matching is a better suggestion engine for this domain than iOS's general-purpose dictionary.
- **No-results page with fuzzy suggestions** — When a player search fails (e.g., "Bergman" due to autocorrect), show "No player found for **X**" with fuzzy match suggestions from `PlayerNameMatcher` instead of creating a stub card. The fuzzy matching infrastructure already exists — needs to be surfaced in the failure path of player card/search resolution.
